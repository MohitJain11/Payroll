...
//                        }
    ...
    private void getAttendanceWhenNoInternet() {
        ...
        String outDate = "";
        String inSyncStatus = "";
        String outSyncStatus = "";
        if (cursor.moveToFirst()) {
            do {
                SimpleDateFormat sdfDate = new SimpleDateFormat("dd-MMM-yyyy");
                String currentDate = sdfDate.format(new Date());
                if (currentDate.equals(cursor.getString(cursor.getColumnIndex("punchDate")))) {
                    String punchStatus = cursor.getString(cursor.getColumnIndex("punchStatus"));
                    if (punchStatus.equals("In")) {
                        inSyncStatus = cursor.getString(cursor.getColumnIndex("isSync"));
                        inStatus = true;
                        inTime = cursor.getString(cursor.getColumnIndex("punchTime"));
                        inDate = cursor.getString(cursor.getColumnIndex("punchDate"));
                    }

                    if (punchStatus.equals("Out")) {
                        outSyncStatus = cursor.getString(cursor.getColumnIndex("isSync"));
                        outStatus = true;
                        outTime = cursor.getString(cursor.getColumnIndex("punchTime"));
                        outDate = cursor.getString(cursor.getColumnIndex("punchDate"));
                    }
                }
...
//                        String[] outTime = returnValue.getString("outTime").split("T");
                    ...
                    out_date_gray.setText(outDate + "\n" + checkOutDateTime(outTime));
                } else {
                    ll_in_button_gray.setVisibility(View.VISIBLE);
                    ll_out_button_gray.setVisibility(View.GONE);
                    ll_in_button.setVisibility(View.GONE);
